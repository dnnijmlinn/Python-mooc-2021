...
@points('3.perakkaisten_summa_versio2')
    ...
    def test_1(self):
        for luku in [3, 4, 5, 6, 7, 8, 9, 10, 15, 21, 33]:
            ...
            with patch('builtins.input', side_effect=[str(luku), AssertionError("Syötettä pyydetään liian monta kertaa.") ], ) as prompt:
                ...
                summattavat = ' + '.join(str(i) for i in range(1, asti) )
    
                odotettu = 'Laskettiin '+summattavat+" = "+ str(summa)
                self.assertTrue(sanitize(odotettu) == sanitize(output), f"Ohjelmasi pitäisi tulostaa\n{odotettu}\nsyötteellä {luku}. Ohjelmasi tulosti\n{output}")
