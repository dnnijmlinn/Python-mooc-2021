...
def hae_nimi(filename: str, word: str):
...
def hae_aika(filename: str, prep_time: int):
    recipes = create_recipes_from_file(filename)
    found_recipes = []
    for i in range(len(recipes)):
        time = int(recipes[i]['prep_time'])
        if time <= prep_time:
            recipe = recipes[i]['name'] + ', ' + 'valmistusaika ' + str(time) + ' min'
            found_recipes.append(recipe)
    return found_recipes

def hae_raakaaine(filename: str, ingredient: str):
    recipes = create_recipes_from_file(filename)
    found_recipes = []
    for i in range(len(recipes)):
